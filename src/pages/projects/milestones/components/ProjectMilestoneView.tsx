...
const ProjectMilestoneView: React.FC = () => {
  ...
  if (!filteredMilestone) {
    return <Typography sx={{ mt: 2 }}>Milestone not found for this project.</Typography>;
  }

  // TODO: Metrics should be fetched from test results associated with this milestone
  // The Milestone type in the spec doesn't include metrics field
  const passed = 0;
  const failed = 0;
  const blocked = 0;
  const untested = 0;
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Typography variant="body2" color="text.secondary">
        ...
        {filteredMilestone.dueDate
          ? new Date(filteredMilestone.dueDate).toLocaleDateString()
          : 'N/A'}
        <strong> Completed:</strong> {filteredMilestone.isCompleted ? 'Yes' : 'No'}
      </Typography>
    </Box>
  );
};
