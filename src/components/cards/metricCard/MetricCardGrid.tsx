...
const MetricCardGrid: React.FC<MetricCardGridProps> = ({ data }) => {
  ...
  return (
    <Grid container spacing={2} justifyContent="center" sx={styles.root}>
      {data.map((item, index) => {
        const trendDiff = item.trend
          ? item.trend.current - item.trend.original
          : null;
        const trendPositive = trendDiff !== null ? trendDiff > 0 : null;
        const trendColor =
          trendPositive && item.trend?.desiredOutcome === "incline"
            ? "green"
            : !trendPositive && item.trend?.desiredOutcome === "decline"
            ? "green"
            : "red";
            ...
            <Box>
              <Box
                sx={{
                  ...
                  p: 1.5,
                  borderRadius: "12px",
                  backgroundColor: `${colorMap[item.color]}22`,
              ...
              {item.trend && (
                <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                  ...
                  >
                    {trendPositive
                      ? `+${Math.abs(trendDiff ?? 0).toFixed(1)}%`
                      : `-${Math.abs(trendDiff ?? 0).toFixed(1)}%`}
                  </Typography>
                </Box>
              )}
